...
class _SwipeToActionWidgetState extends State<SwipeToActionWidget> {
  ...
  double _getInitialDragX() {
    if (widget.doubleSide) {
      return (widget.width - widget.height) / 2; // default width/height, will be replaced by actual
    }
    return widget.swipeRight ? 0.0 : widget.width - widget.height;
  }
  ...
  Widget build(BuildContext context) {
    ...
    final minX = 0.0;
    final maxX = width - height;
    final centerX = (width - height) / 2;
    final percent = widget.doubleSide
        ? (_dragX - centerX) / centerX
        : widget.swipeRight
        ? (_dragX - minX) / (maxX - minX)
        : (maxX - _dragX) / (maxX - minX);
    Color bgColor = widget.backgroundColor;
    IconData icon = widget.doubleSide
        ? Icons.drag_handle
        : (widget.swipeRight ? Icons.arrow_forward_ios_rounded : Icons.arrow_back);
    if (widget.doubleSide) {
      if (percent < -0.2) {
    ...
    if (!widget.doubleSide) {
      ...
      if (t <= 0.5) {
        ...
        offset2 = (1 - fade2) * headingHeight;
      }
    } else {
      // Double side: keep previous logic
      if (percent >= 0) {
        fade1 = 1.0 - percent.clamp(0, 1).toDouble();
        fade2 = percent.clamp(0, 1).toDouble();
        offset1 = -percent.clamp(0, 1).toDouble() * headingHeight;
        offset2 = (1 - percent.clamp(0, 1).toDouble()) * headingHeight - headingHeight;
      } else {
        fade1 = -percent.clamp(-1, 0).toDouble();
        fade2 = 1.0 + percent.clamp(-1, 0).toDouble();
        offset1 = (1 + percent.clamp(-1, 0).toDouble()) * -headingHeight + headingHeight;
        offset2 = -percent.clamp(-1, 0).toDouble() * headingHeight;
      }
    }
        ...
        Positioned(
          ...
          child: GestureDetector(
            onHorizontalDragUpdate: (details) {
              setState(() {
                double newDragX = _dragX + details.delta.dx;
                if (widget.doubleSide) {
                  _dragX = newDragX.clamp(minX, maxX);
            ...
            onHorizontalDragEnd: (details) {
              ...
              } else if (widget.swipeRight && _dragX >= maxX - 10) {
                setState(() => _completed = true);
                Future.delayed(const Duration(milliseconds: 200), () {
                  widget.onSwipe();
                  _reset();
                });
              } else if (!widget.swipeRight && _dragX <= minX + 10) {
                setState(() => _completed = true);
                Future.delayed(const Duration(milliseconds: 200), () {
                  widget.onSwipe();
                  _reset();
